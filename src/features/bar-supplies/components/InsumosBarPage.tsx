...
import { MdLocalBar, MdEdit, MdDelete, MdSearch, MdAdd, MdSwapVert } from "react-icons/md";
import { useInsumosBar } from "../hooks/useInsumosBar";
import { authClient } from "@/config/authClient";
import {
  UnidadInsumo, TipoMovimiento, MotivoEntrada, MotivoSalida,
} from "../types";
import type { InsumoBar, CreateInsumoBar, UpdateInsumoBar, CreateMovimientoBar } from "../types";
...
function insumoToForm(i: InsumoBar): InsumoFormState {
  return {
...
export default function InsumosBarPage() {
  ...
  const [editingInsumo, setEditingInsumo] = useState<InsumoBar | null>(null);
  const [insumoForm, setInsumoForm] = useState<InsumoFormState>(emptyInsumoForm);
  const [savingInsumo, setSavingInsumo] = useState(false);

  const [movModal, setMovModal] = useState(false);
  const [movTarget, setMovTarget] = useState<InsumoBar | null>(null);
  const [movForm, setMovForm] = useState<MovForm>(emptyMovForm);
  const [savingMov, setSavingMov] = useState(false);

  const [deleting, setDeleting] = useState(false);

  if (loading) return <div className="flex items-center justify-center min-h-[60vh] text-text-muted text-sm">Cargando insumos de bar...</div>;
  ...
  const openEdit = (i: InsumoBar) => { setEditingInsumo(i); setInsumoForm(insumoToForm(i)); setInsumoModal(true); };
  ...
  const handleDelete = async (i: InsumoBar) => {
  ...
  // ── Movimiento ──
  const openMov = (i: InsumoBar) => { setMovTarget(i); setMovForm(emptyMovForm); setMovModal(true); };
  ...
  const handleMovSubmit = async (e: React.FormEvent) => {
    ...
    setSavingMov(true);
    try {
      const payload: CreateMovimientoBar = {
  ...
  return (
    <>
      <PanelHeader
        title="Insumos de Bar"
        subtitle="Control de stock e inventario del bar"
      ...
      {/* ── Insumo Modal ── */}
      {isAdmin && (
        <Modal isOpen={insumoModal} onClose={closeInsumoModal} title={editingInsumo ? "Editar Insumo" : "Nuevo Insumo de Bar"}>
          <form onSubmit={handleInsumoSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <InputField label="Código *" value={insumoForm.codigo} onChange={(e) => setInsumoForm((f) => ({ ...f, codigo: e.target.value }))} placeholder="BAR-001" required />
            ...
            {editingInsumo && (
              <div className="flex items-center gap-3">
                <input type="checkbox" id="activo-bar" checked={insumoForm.activo} onChange={(e) => setInsumoForm((f) => ({ ...f, activo: e.target.checked }))} className="w-4 h-4 accent-primary" />
                <label htmlFor="activo-bar" className="text-sm text-text-primary">Activo</label>
              </div>
            )}
