          ...
          {!isEditing && (
            <div className="space-y-4">
              <div className="relative">
                ...
                {reservaSeleccionada && (
                  <div className="mt-2 bg-primary/5 border border-primary/20 rounded-xl px-4 py-3 grid grid-cols-2 gap-x-4 gap-y-1">
              ...
              <div>
                ...
                {/* Consumos del folio */}
                {loadingConsumos && <div className="mt-2 text-center py-3 text-text-muted text-xs">Cargando consumos...</div>}
                {consumos && (consumos.productos.length > 0 || consumos.servicios.length > 0) && (
                  <div className="mt-2 border border-border rounded-xl overflow-hidden">
                    <div className="bg-bg-card px-3 py-2 font-semibold text-text-muted uppercase border-b border-border">Consumos del Folio</div>
                    ...
                    {consumos.total != null && (
                      <div className="flex items-center justify-between px-3 py-2 bg-primary/5 border-t border-primary/20">
                        ...
                        <span className="font-bold text-primary">S/ {Number(consumos.total).toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                )}
