              ...
              {Object.entries(CONDICION_LABELS).map(([key, label]) => (
                <div key={key} className="flex items-center gap-2 text-xs">
                  <div className={cn("w-2.5 h-2.5 rounded-full shadow-sm", {
                    "bg-success": key === "BUENO",
                    "bg-warning": key === "REGULAR",
                    "bg-danger": key === "DANADO",
                    "bg-text-muted": key === "FALTANTE",
                  })} />
          ...
          <div className="bg-paper-lightest rounded-2xl w-full max-w-md border border-border-light/50 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-accent-primary to-accent-light px-6 py-4">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-paper-lightest font-display">Detalle del Mueble</h2>
            ...
            <div className="p-6 space-y-5">
              <div className="flex items-center gap-4">
                <div className={cn("w-14 h-14 rounded-xl flex items-center justify-center",
                  selectedMueble.condicion === "BUENO" ? "bg-success-bg" :
                  selectedMueble.condicion === "REGULAR" ? "bg-warning-bg" :
                  selectedMueble.condicion === "DANADO" ? "bg-danger-bg" : "bg-bg-tertiary"
                )}>
                  <svg className={cn("w-7 h-7",
                    selectedMueble.condicion === "BUENO" ? "text-success" :
                    selectedMueble.condicion === "REGULAR" ? "text-warning" :
                    selectedMueble.condicion === "DANADO" ? "text-danger" : "text-text-muted"
                ...
                <div>
                  <p className="text-xl font-bold font-display">{selectedMueble.nombre}</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-paper-medium/20 rounded-xl p-3"><p className="text-text-muted text-xs">Condición</p><span className={`inline-block px-2 py-1 text-xs font-semibold rounded-full ${selectedMueble.condicion === "BUENO" ? "bg-success-bg text-success" : selectedMueble.condicion === "REGULAR" ? "bg-warning-bg text-warning" : selectedMueble.condicion === "DANADO" ? "bg-danger-bg text-danger" : "bg-bg-tertiary text-text-muted"}`}>{CONDICION_LABELS[selectedMueble.condicion]}</span></div>
