      ...
      <div className="p-4 pt-5">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-text-darkest text-sm">{tipo.nombre}</p>
              <p className="text-xs text-text-muted line-clamp-1">{tipo.descripcion || "Sin descripción"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-3 border-t border-border-light/30">
          <span className={cn(
            ...
            tipo.tiene_ducha ? "bg-emerald-100 text-emerald-700" : "bg-stone-100 text-stone-500"
          )}>
          ...
          <span className={cn(
            "px-2 py-1 text-[10px] font-semibold rounded-full flex items-center gap-1",
            tipo.tiene_banio ? "bg-emerald-100 text-emerald-700" : "bg-stone-100 text-stone-500"
          )}>
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
